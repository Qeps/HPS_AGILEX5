...
module HPS_QSYS_altera_merlin_width_adapter_1961_ret2bzy 
#(
...
( 
    ...
    reg [BYTE_CNT_W-1:0]        old_quantized_byte_cnt_field;
    //AXI5Lite Poison 
    ...
    reg [ADDRESS_W-1:0]         out_address_field_reg1;
    reg [ADDRESS_W-1:0]         out_address_field_reg2;
    reg                         out_valid_reg1;
    reg                         out_cmpr_read;
    ...
    reg [OUT_SEGMENT_W/2-1:0]     data_array_tmp1;
    reg [OUT_SEGMENT_W-1:0]     data_array_tmp2;
    reg [OUT_SEGMENT_W-1:0]     data_array_tmp2_reg;
    ...
    reg [IN_ST_DATA_W-1:0]    inp_data_reg;
    reg [IN_ST_DATA_W-1:0]    inp_data;
    reg inp_startofpacket;
    reg inp_endofpacket;
    reg [ST_CHANNEL_W-1:0] inp_channel;
    reg inp_startofpacket_reg;
    reg inp_endofpacket_reg;
    reg [ST_CHANNEL_W-1:0] inp_channel_reg;
    reg [7:0] count_remaining_bytes='0;
    reg packet_state;
    reg ready_state;
    reg ready_next_state='0;
    reg packet_next_state='0;
    localparam GENERATE_VALID=1'b0;
    localparam BACKPRESSURE=1'b1;
    localparam PACKET_END=1'b0;
    localparam PACKET_IN_TRANSIT=1'b1;
    
    if (SYNC_RESET == 0) begin : async_rst3
       always @(posedge clk, posedge reset) begin
          if (reset) begin
              inp_data_reg <= 'b0;
              inp_startofpacket_reg <= 'b0;
              inp_endofpacket_reg <= 'b0;
              inp_channel_reg <= '0;
              ready_state  <= GENERATE_VALID;
              packet_state  <= PACKET_IN_TRANSIT;
          end else begin
              inp_data_reg <= inp_data;
              inp_startofpacket_reg <= inp_startofpacket;
              inp_endofpacket_reg <= inp_endofpacket;
              inp_channel_reg <= inp_channel;
              ready_state  <= ready_next_state;
    ...
    always @* begin
        //if(in_valid && ~|count_remaining_bytes && (in_data[IN_PKT_BURST_SIZE_H :IN_PKT_BURST_SIZE_L ]==clogb2(IN_NUMSYMBOLS))) begin
        if(RESPONSE_PATH && ENABLE_AXI4) begin
            if(in_valid && ready_state==GENERATE_VALID) begin
               inp_data  = in_data;
               inp_channel = in_channel;
               inp_startofpacket=in_startofpacket;
               inp_endofpacket=in_endofpacket;
            //end else if(in_valid && in_ready) begin
            //   inp_data  = in_data;
            end else begin
               inp_data = inp_data_reg;
               inp_channel = inp_channel_reg;
               inp_startofpacket=inp_startofpacket_reg;
               inp_endofpacket=inp_endofpacket_reg;
            end
        end else begin
            inp_data  = in_data;
            inp_channel = in_channel;
        ...
        in_size_field            = inp_data[IN_PKT_BURST_SIZE_H :IN_PKT_BURST_SIZE_L ];
        address_from_packet      = inp_data[IN_PKT_ADDR_H       :IN_PKT_ADDR_L       ];
        in_burst_type_field      = inp_data[IN_PKT_BURST_TYPE_H :IN_PKT_BURST_TYPE_L ];
        in_data_field            = inp_data[IN_PKT_DATA_H       :IN_PKT_DATA_L       ];
        in_byteen_field          = inp_data[IN_PKT_BYTEEN_H     :IN_PKT_BYTEEN_L     ];
        in_byte_cnt_field        = inp_data[IN_PKT_BYTE_CNT_H   :IN_PKT_BYTE_CNT_L   ];
        in_cmpr_read             = inp_data[IN_PKT_TRANS_COMPRESSED_READ];
        in_write                 = inp_data[IN_PKT_TRANS_WRITE];
        in_lock_field            = inp_data[IN_PKT_TRANS_EXCLUSIVE];
        in_response_status_field = inp_data[IN_PKT_RESPONSE_STATUS_H :IN_PKT_RESPONSE_STATUS_L];
        if (ROLE_BASED_USER || ENABLE_AXI5) begin
           in_poison_field          = inp_data[IN_PKT_POISON_H : IN_PKT_POISON_L ];
           in_datachk_field         = inp_data[IN_PKT_DATACHK_H : IN_PKT_DATACHK_L]; 
           in_sai_field             = inp_data[IN_PKT_SAI_H     : IN_PKT_SAI_L];
        end
        else begin
           in_poison_field          = '0;
           in_datachk_field         = '0; 
       ...
       if (ROLE_BASED_USER)begin
           in_user_data_field       = inp_data[IN_PKT_USER_DATA_H:IN_PKT_USER_DATA_L];            
           in_addrchk_field         = inp_data[IN_PKT_ADDRCHK_H : IN_PKT_ADDRCHK_L];
       end
       else begin
       ...
       if(ENABLE_AXI5)begin
          in_awakeup_field = inp_data[IN_PKT_AWAKEUP];
          in_atrace_field = inp_data[IN_PKT_ATRACE];
          in_trace_field  = inp_data[IN_PKT_TRACE];
        end
       else begin
           in_awakeup_field       = '0;
           in_atrace_field        = '0;
        ...
        in_awatop_field        = inp_data[IN_PKT_AWATOP_H:IN_PKT_AWATOP_L];
        in_awstashnid_field    = inp_data[IN_PKT_AWSTASHNID_H:IN_PKT_AWSTASHNID_L];
        in_awstashniden_field  = inp_data[IN_PKT_AWSTASHNIDEN];
        in_awstashlpid_field   = inp_data[IN_PKT_AWSTASHLPID_H:IN_PKT_AWSTASHLPID_L];
        in_awstashlpiden_field = inp_data[IN_PKT_AWSTASHLPIDEN];
        in_awmmusecsid_field   = inp_data[IN_PKT_MMUSECSID];
        in_awmmusid_field      = inp_data[IN_PKT_MMUSID_H:IN_PKT_MMUSID_L]; 
        in_dataless_txn_field  = inp_data[IN_PKT_DATALESS] ;
    end
    ...
    generate 
        if (FIRST_EXISTS) begin : gen_field
            always @* begin
                in_first_field = inp_data[IN_FIRST_H:IN_FIRST_L];
            end
        end else begin
            always @* begin
                in_first_field = '0;
        ...
        if (MID_EXISTS) begin
            always @* begin
                in_mid_field = inp_data[IN_MID_H:IN_MID_L];
            end
        end else begin
            always @* begin
                in_mid_field = '0;
        ...
        if (LAST_EXISTS) begin
            always @* begin
                in_last_field = inp_data[IN_LAST_H:IN_LAST_L];
            end
        end
    endgenerate
    ...
    generate
      ...
      if (IN_NUMSYMBOLS > OUT_NUMSYMBOLS)  begin
        ...
        reg out_valid_ctrl;
        reg out_valid_ctrl_reg;
        reg [BURST_SIZE_W-1:0]  in_ori_size_field;
        always @* begin
            in_ori_size_field = inp_data[IN_PKT_ORI_BURST_SIZE_H :IN_PKT_ORI_BURST_SIZE_L ];
        end
        wire in_size_less_than_ori_size = (in_size_field < in_ori_size_field) ? 1 : 0;
        wire incr_burst_type = (in_burst_type_field == 2'b01) ? 1 : 0;
         ...
         wire [31:0] int_ratio_minus_1 ;//= (in_size_less_than_ori_size && incr_burst_type) ? 0 : ((cmd_burst_size / OUT_NUMSYMBOLS) - 1);
         wire [31:0] address_offset ; //Keeps a track of unaligned address , used to part select the response
         wire [clogb2(RATIO)-1:0] sized_ratio_minus_1 ;
        
        if(ENABLE_AXI4) begin 
            assign int_ratio_minus_1 = (in_size_less_than_ori_size && incr_burst_type) ? 0 : ((cmd_burst_size / OUT_NUMSYMBOLS) - 1);
            assign address_offset = (address_from_packet%IN_NUMSYMBOLS) >> clogb2(OUT_NUMSYMBOLS); //Keeps a track of unaligned address , used to part select the response
            assign sized_ratio_minus_1 = (|address_offset && (inp_startofpacket && incr_burst_type)) ?  int_ratio_minus_1[clogb2(RATIO)-1:0]-address_offset[clogb2(RATIO)-1:0] : int_ratio_minus_1[clogb2(RATIO)-1:0] ;
         end else begin
            assign int_ratio_minus_1 = (cmd_burst_size / OUT_NUMSYMBOLS) - 1;
            assign sized_ratio_minus_1 = int_ratio_minus_1[clogb2(RATIO)-1:0] ;
         end
         ...
         //reg ready_state;
         //reg ready_next_state;
         //reg packet_state;
         //reg packet_next_state;

         reg [FIRST_W-1:0] out_first_field_reg; 
         reg [MID_W-1:0] out_mid_field_reg;          
         reg [LAST_W-1:0] out_last_field_reg;            
         reg out_cmpr_read_reg;           
         reg out_lock_field_reg;            
         reg [BURST_TYPE_W-1:0] out_burst_type_field_reg;
         reg [RESPONSE_STATUS_W-1:0] out_response_status_field_reg;

         localparam IDLE=2'b00;
         localparam PACKET_STARTED=2'b01;
         localparam HOLD_OUTPUT_SEL=2'b10;
         localparam PACKET_ENDED=2'b11;

         assign single_response_expected = (RESPONSE_PATH && ((only_one_segment_asserted && inp_startofpacket && inp_endofpacket) || in_write));      
         ...
         if (SYNC_RESET == 0)  begin :async_rst0
            always @(posedge clk, posedge reset) begin
               if (reset) begin
                  ...
                  use_reg      <= '0;
                  endofpacket_reg <= '0;
                  data_array_tmp2_reg<='0;
                  out_valid_ctrl_reg<='0;
               end else begin
                  // If we're not working on a wide datum, 
                  // then wait until one arrives.
                  if (~use_reg) begin
                     endofpacket_reg <= inp_endofpacket;
                    
                    if(ENABLE_AXI4) begin: axi4_enabled_data_arrived_check
                     if (in_valid && out_ready && !in_cmpr_read && (cmd_burst_size > OUT_NUMSYMBOLS) && (!single_response_expected && |sized_ratio_minus_1) && !out_valid_ctrl) begin
                        // Data has arrived!
                  ...
                  data_array_tmp2_reg<=data_array_tmp2;
                  out_valid_ctrl_reg<=out_valid_ctrl;
               end // if (posedge clk)
         ...
         always @* begin
            ...
            //   not ready.
            in_ready = 0;
        	packet_next_state = packet_state;
			ready_next_state = ready_state;
			out_valid_ctrl = 0;
           if(ENABLE_AXI4 && incr_burst_type && (in_size_field==clogb2(IN_NUMSYMBOLS) || in_size_field=='0 )) begin: axi4_enabled_ctrl_ready_valid
           
           if ((((cmd_burst_size <= OUT_NUMSYMBOLS) || count == 1 || in_cmpr_read ) || ~|sized_ratio_minus_1) && !in_size_less_than_ori_size) begin
               if(ready_state==BACKPRESSURE)
                   in_ready = 0;
               else
                   in_ready = out_ready;
               if(out_ready)
                   ready_next_state=GENERATE_VALID;

           end else if (in_size_less_than_ori_size) begin
               case(ready_state)
                GENERATE_VALID:begin
                                in_ready=1;
                                if((count_remaining_bytes%OUT_NUMSYMBOLS==OUT_NUMSYMBOLS-1) && in_valid && out_ready) begin
                                    out_valid_ctrl = 1;
                                    ready_next_state=GENERATE_VALID;
                                end else if((count_remaining_bytes%OUT_NUMSYMBOLS==OUT_NUMSYMBOLS-1) && in_valid && in_ready) begin
                                    out_valid_ctrl = 1;
                                    ready_next_state=BACKPRESSURE;
                                end else begin
                                    ready_next_state=GENERATE_VALID;
                                    out_valid_ctrl = 0;
                                end
                ...
                default:       begin
                                ready_next_state=ready_state; 
                                out_valid_ctrl = out_valid_ctrl_reg;
		               end              
               endcase
           end
            ...
            else if(in_size_less_than_ori_size)// || |count_remaining_bytes)
               out_valid                 = out_valid_ctrl;
            else
               out_valid                 = in_valid; 
           end else begin
            ...
            if(ENABLE_AXI4) 
                out_channel               = inp_channel;
            else
                out_channel               = in_channel;

            out_startofpacket         = inp_startofpacket;
            out_endofpacket           = 0;
         if(ENABLE_AXI4 && (in_size_less_than_ori_size || ready_state==BACKPRESSURE || out_valid_ctrl_reg)) begin
          case(packet_state)
            PACKET_END:begin
                            out_endofpacket = 1;
                            out_startofpacket = 0;
                            if(out_ready) begin
                                packet_next_state = PACKET_IN_TRANSIT;
                            end
            end
            PACKET_IN_TRANSIT:begin
                                out_endofpacket = 0;
                                out_startofpacket = inp_startofpacket;
                                if(out_valid_ctrl && out_ready) begin
                                    out_endofpacket = 1;
                                    out_startofpacket = 0;
                                end else if(out_valid_ctrl && in_valid && in_ready && inp_endofpacket) begin
                                    packet_next_state = PACKET_END;
                                    out_endofpacket = 1;
                                    out_startofpacket = 0;
                                end
            end
           ...
           endcase
            ...
            if (cmd_burst_size <= OUT_NUMSYMBOLS) begin
                out_endofpacket = inp_endofpacket;
                if(ENABLE_AXI4) begin
                    if(packet_state == PACKET_END) begin
                        out_endofpacket = 1;
                        out_startofpacket = 0;
                    end
                end
            ...
            if (!CONSTANT_BURST_SIZE) begin
              ...
              if (ENABLE_AXI4 && out_valid_ctrl) begin
                out_data_field         = data_array_tmp2;
              end else begin
      ...
      if(ENABLE_AXI4) begin: axi4_enabled_couny_remaining_bytes  
        always @(posedge clk) begin
           if(in_size_less_than_ori_size && count_remaining_bytes<IN_NUMSYMBOLS) begin
               if(in_valid && in_ready) begin
                count_remaining_bytes <= count_remaining_bytes + 1'b1;
               end else begin
                count_remaining_bytes <= count_remaining_bytes;
               end
           end else if(!in_valid || !in_ready)begin
               count_remaining_bytes <= count_remaining_bytes;
           end else begin
               count_remaining_bytes <= 'b0;
           end
         ...
         if (!CONSTANT_BURST_SIZE) begin
             integer ibyte;
             always @* begin
		 data_array_tmp2=data_array_tmp2_reg;
                 if (in_size_less_than_ori_size && ENABLE_AXI4 && incr_burst_type) begin
                     if((in_valid && in_ready)) begin
                        data_array_tmp1 = in_data_field[(in_address_field[NW_BITFORSELECT_R-1:0])*8+:8];
                        data_array_tmp2[(in_address_field[NW_BITFORSELECT_R-1:0]%OUT_NUMSYMBOLS)*8+:8] = data_array_tmp1;
                     end else begin
                        data_array_tmp2=data_array_tmp2_reg;
                     end
                    for(ibyte=0; ibyte<RATIO; ibyte=ibyte+1) begin: mux_mapping_to_zero
                        data_array[ibyte] = '0;
                        byteen_array[ibyte] = '0;
                    end
                 end else begin
      ...
      if (OUT_NUMSYMBOLS > IN_NUMSYMBOLS)  begin
         ...
         reg [ADDRESS_W-1:0]     p1_address_offset;
         reg [ADDRESS_W-1:0]     p1_address_offset_reg;
         ...
         reg  [4:0]              no_of_ones                    = '0;
         reg  [4:0]              no_of_ones_tmp                = '0;
         wire [31:0]             int_encoded_burstsize         = NW_BITFORSELECT_R; //NW_BITFORSELECT_R is the log2 of IN_NUMSYMBOLS
         wire [BURST_SIZE_W-1:0] encoded_burstsize;
         wire quant_byte_cnt_equals_out_numsymb                = (quantized_byte_cnt_field==OUT_NUMSYMBOLS);
         wire p0_burst_type_incr                               = p0_burst_type_field[0:0];
         wire byte_cnt_greater_than_out_numsymb         = (|p0_byte_cnt_field[BYTE_CNT_W-1:NW_BITFORSELECT_L+1]);
         wire large_packet = (byte_cnt_greater_than_out_numsymb && !p1_startofpacket && p0_burst_type_incr) ? 1 : 0; 
         reg flag;
         reg flag_reg;

         if(ENABLE_AXI4) begin
            assign encoded_burstsize = (quant_byte_cnt_equals_out_numsymb) ? '0 : int_encoded_burstsize[BURST_SIZE_W-1:0];     
         end else begin
            assign encoded_burstsize = int_encoded_burstsize[BURST_SIZE_W-1:0];     
         end
        ...
        assign aligned_addr     = (~|p0_address_field[ALIGNED_BITS_L:0]);
        assign aligned_byte_cnt = (~|p0_byte_cnt_field[ALIGNED_BITS_L:0]);
        always @* begin
        if(ENABLE_AXI4) begin
            if ((RESPONSE_PATH == 0) && (PACKING == 1) && p0_burst_type_incr && byte_cnt_greater_than_out_numsymb) begin // if this is AXI then checking on aligned (not for burst type WRAP)
               unaligned_read   = p0_cmpr_read & (~aligned_addr || ~aligned_byte_cnt);
            end else begin
               unaligned_read   = '0;
            end
        end else begin
        ...
        #(
            ...
            .SYNC_RESET  (SYNC_RESET),
            .USED_IN_WIDTH_ADAPTER  (1'b1) // set to 1 because we know this is width adapter
        ) uncompressor (
            .clk                  (clk),
            .reset                (reset),
         ...
         if (SYNC_RESET == 0) begin : async_rst2
            always @(posedge clk, posedge reset) begin
               if (reset) begin
                  ...
                  old_quantized_byte_cnt_field <= '0;
                  out_valid_reg1 <= 0;
                  flag_reg<=0;
                  out_address_field_reg2<='0;
               end else begin
                  ...
                  if (p1_valid && (out_ready || ~out_valid)) begin
                    ...
                    old_quantized_byte_cnt_field<=quantized_byte_cnt_field;
                  end // if (p1_valid && (out_ready || ~out_valid))
                  out_valid_reg1 <= out_valid && out_ready;
                  flag_reg <= flag;
                  out_address_field_reg2 <= out_address_field_reg1;
		  p1_address_offset_reg <= p1_address_offset;
               end // if (posedge clk)
            end // always @ (clk, reset)
         end // async_rst2
         ...
         always @* begin
            ...
            if (PACKING == 1) begin
                if (p1_endofpacket || (p1_push_data_to_output == '1)) 
                    out_valid = p1_valid;
            end else begin
                out_valid = p1_valid;
            end
            ...
            if (p1_cmpr_read && (p1_byte_cnt_field > IN_NUMSYMBOLS)) begin
                ...
                if (PACKING == 1) begin  // byteenable only affect on command path
                    ...
                    if(quant_byte_cnt_equals_out_numsymb && ENABLE_AXI4 && p0_burst_type_incr && large_packet) 
                        out_byteen_field[p1_shift_correct_ouput_segments +: 1] = 1;
                    else
            ...
            out_address_field_reg1 = (out_valid_reg1 || unc_src_startofpacket) ? p1_address_field:out_address_field_reg2; 
            p1_address_offset = p1_address_offset_reg;
            if (RESPONSE_PATH == 0) begin
